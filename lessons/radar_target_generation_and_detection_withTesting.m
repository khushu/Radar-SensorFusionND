...
signal_cfar2 = zeros(Nr/2,Nd);

%For testing
testMatrix  = zeros(Nr/2,Nd);
...
for j = 1:(Nr/2)-(2*(Gd+Td))
    ...
    for i = 1:(Nd-2*(Gr+Tr))        
        ...
        %For testing only        
        %Not sure why this is not working, the entire block becomes one
        %instead of guard cells
        %TrainingXIndex = [ TrainingXIndexA TrainingXIndexB TrainingXIndexC];
        %TrainingYIndex = [ TrainingYIndexA TrainingYIndexB TrainingYIndexC];
        %testMatrix(TrainingYIndex,TrainingXIndex) = 1; 
        
        %For testing only
%         testMatrix(TrainingYIndexA,TrainingXIndexA) = 1;
%         testMatrix(TrainingYIndexB,TrainingXIndexB) = 1;
%         testMatrix(TrainingYIndexC,TrainingXIndexC) = 1;        
%         testMatrix = zeros(Nr/2,Nd);
        ...
        %3 sum should be equal to noise 
        
        s1 = sum(db2pow(RDM(j:j+2*Td+2*Gd, i:i+2*Tr+2*Gr)),'all');
        s2 = sum(db2pow(RDM(j+Td:j+Td+2*Gd, i+Tr:i+Tr+2*Gr)),'all');    
        noise_level = s1 - s2;
        ...
        threshold = noise_level/TotalCellCount;
        threshold2 = TrainingSum/TotalCellCount;
...
%         
        threshold2 = pow2db(threshold2) + Offset;
        threshold2 = db2pow(threshold2);
...
%         end
        ...
        signal2 = db2pow(RDM(CutYIndex, CutXIndex));
...
%         signal_cfar(i+Tr+Gr,j+Td+Gd) = signal; 
        signal_cfar2(CutYIndex,CutXIndex) = signal2; 
...
figure,surf(doppler_axis,range_axis,signal_cfar2);
colorbar;
